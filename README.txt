...
### 도메인
- [ ] 이름을 받아서 자동차를 만들 수 있다.
    - [ ] 빈 문자열은 허용하지 않는다.
    - [ ] 5자 이하의 문자열만 허용한다.
- [ ] 자동차를 조건에 따라서 전진시킨다.(0에서 9 사이에서 random 값을 구한 후, random 값이 4 이상일 경우 전진하고, 3 이하의 값이면 전진하지 않는다)
- [ ] 시도 횟수만큼 자동차들의 전진을 시도한다.
- [ ] 자동차들 중 가장 앞서 있는 자동차들을 찾는다.
